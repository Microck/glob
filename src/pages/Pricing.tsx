...
const Pricing = () => {
  ...
  const [glowSize, setGlowSize] = useState(60);
  ...
  const handleCopyConfig = () => {
    const config = {
      glowIntensity, glowSize, gradientIntensity, patternOpacity, patternSize, patternSpacing,
      ...
      <div className="flex flex-col items-center w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl relative z-10">
          ...
          <div 
            ...
            style={{ 
              background: `
                radial-gradient(circle at ${glowX}% ${glowY}%, ${glowColor}${Math.floor(glowIntensity * 255).toString(16).padStart(2, '0')} 0%, transparent ${glowSize}%),
        ...
        <div className="mt-8 p-6 bg-surface border-3 border-muted grid grid-cols-2 md:grid-cols-6 gap-x-8 gap-y-4 z-20 shadow-brutal max-w-7xl w-full">
          ...
          <div className="flex flex-col gap-1">
            <span className="font-ui text-[10px] text-muted uppercase tracking-widest">Glow Int./Size</span>
            <div className="flex gap-2">
              <input type="range" min="0" max="0.5" step="0.01" value={glowIntensity} onChange={(e) => setGlowIntensity(Number(e.target.value))} className="accent-active w-1/2" title="Intensity" />
              <input type="range" min="10" max="100" step="1" value={glowSize} onChange={(e) => setGlowSize(Number(e.target.value))} className="accent-active w-1/2" title="Size" />
            </div>
          </div>
          <div className="flex flex-col gap-1">
